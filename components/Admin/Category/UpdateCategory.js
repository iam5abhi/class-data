...
export default function UpdateCategory({id,setOpen,open,getQueriesData}) {
  ...
  const handleSubmit =(event)=>{
      ...
      body: JSON.stringify({id:id,name:formData}),
      }).then((res) => {
        getQueriesData() 
        setOpen(false)
      })
  ...
  const getContactData = () => {
    fetch("/api/category/get-singlecategory", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((data) => { setFormData(data.name) })
                      ...
                      <div className="relative flex flex-col flex-auto min-w-0 mt-2 p-4 break-words border-0 shadow-blur rounded-2xl bg-white/80 bg-clip-border mb-4 draggable " draggable="true">
                        <form onSubmit={handleSubmit}> 
                              <div >
                                ...
                                <input type="text" value={formData} onChange={()=>setFormData(event.target.value)} id="name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder='Enter Name' required/>
                              </div>
